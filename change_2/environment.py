...
class CreateClusEnv(Env):    
    ...
    def cluster_head_array(self):
        ch_array=[]
        size = data_state.car_count
        for index in range(size):
            velocity =data_state.Speed_result_list.pop()
            excel_reader_instance1 = Speed_state.ExcelReader()
            excel_reader_instance1.start_row += 1
            position, direction = self.random_position(np.array([data_state.X_result_list.pop(), data_state.Y_result_list.pop()]),'')
            excel_reader_instance2 = X_state.ExcelReader()
            excel_reader_instance2.start_row += 1
            excel_reader_instance3 = Y_state.ExcelReader()
            excel_reader_instance3.start_row += 1
            # VEHICLE PARA (velocity, direction, dist_ch, dist_fg, position)
            vh = Vehicle(velocity, direction, 10, 10, position)
            core = random.choice([4, 8])
            ram = random.choice([8, 16])
            cpu_util = data_state.Cpu_result_list.pop()
            excel_reader_instance4 = CPU_state.ExcelReader()
            excel_reader_instance4.start_row += 1
